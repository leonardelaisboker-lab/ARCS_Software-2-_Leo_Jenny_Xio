...
def find_objects(objects_pcd, cube_pcd):
    ...
    # Less aggressive: only downsample if >5 million points
    if len(objects_pcd.points) > 5000000:  # 5 million threshold
        print("⚡ Downsampling for faster clustering...")
        voxel_size = 0.002  # 2mm voxels (less aggressive than 5mm!)
        ...
        print("✓ Point count OK, using full resolution")
    ...
    if len(objects_pcd_downsampled.points) < 1000000:
        clustering_eps = 0.01  # 10mm for smaller clouds
    elif len(objects_pcd_downsampled.points) < 3000000:
        clustering_eps = 0.015  # 15mm for medium clouds
    else:
        clustering_eps = 0.02  # 20mm for large clouds
