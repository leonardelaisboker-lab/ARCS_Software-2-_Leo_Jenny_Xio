...
- Calibration cube (60×60×60mm with ArUco markers, 50×50mm)
...
def find_objects(objects_pcd, cube_pcd):
    ...
    # Only downsample if REALLY necessary (>10 million points)
    # And keep at least 50% of points!
    if len(objects_pcd.points) > 10000000:  # 10 million threshold
        print("⚡ Downsampling to reasonable size...")
        # Target: reduce to ~5 million points max
        # Calculate voxel size to achieve this
        target_points = 5000000
        ratio = len(objects_pcd.points) / target_points
        voxel_size = 0.001 * (ratio ** (1/3))  # Scale voxel size with cube root
        
        objects_pcd_downsampled = objects_pcd.voxel_down_sample(voxel_size=voxel_size)
        reduction = 100 * (1 - len(objects_pcd_downsampled.points) / len(objects_pcd.points))
        print(f"✓ Reduced to {len(objects_pcd_downsampled.points)} points")
        print(f"  Voxel size: {voxel_size*1000:.2f}mm, Reduction: {reduction:.1f}%")
    else:
        print("✓ Point count OK, using full resolution (no downsampling)")
        objects_pcd_downsampled = objects_pcd
    
    print(f"\nClustering {len(objects_pcd_downsampled.points)} points...")
    if len(objects_pcd_downsampled.points) > 1000000:
        print("This may take 5-10 minutes... ☕")
    
    # DBSCAN clustering with reasonable parameters
    clustering_eps = 0.01  # 10mm - consistent for all sizes
